...
def print(*args, **kwargs):
    pass  # Do not log remote targets or exception payloads.
...
MAX_PROXY_CONNECTIONS = min(64, parse_positive_int(os.environ.get("LOCAL_PROXY_MAX_CONNECTIONS"), 64))
...
def get_proxy_credentials() -> tuple[str | None, str | None]:
    cfg = security.load_credentials()
    return cfg['proxy_username'], cfg['proxy_password']
...
def check_credentials(username: str | None, password: str | None) -> bool:
    ...
    if expected_user is None or expected_pass is None:
        return False
    return security.credential_match(username or "", expected_user) and security.credential_match(password or "", expected_pass)

def dns_query_over_vpn(host: str, qtype: int, dns_server: str, timeout: float) -> str | None:
    sock = None
    try:
        ...
        vpn_runtime.bind_socket(sock)
        # Connected UDP accepts replies only from the selected resolver endpoint.
    ...
    try:
        if (len(resp) < 12 or resp[:2] != tx_id or not resp[2] & 0x80
                or resp[2] & 0x7A or resp[4:6] != b"\x00\x01"
                or resp[12:12 + len(qname) + 4] != qname + qtype_qclass):
            return None
        rcode = resp[3] & 0x0F
        if rcode != 0:
            return None
...
def resolve_dns_over_vpn(host: str, dns_server: str = "8.8.8.8", timeout: float = 3.0) -> str | None:
    try:
    ...
    try:
        ...
        pass
    return dns_query_over_vpn(host, 1, dns_server, timeout)

def create_connection(address: tuple[str, int], timeout: float = 20) -> socket.socket:
    host, port = address
    resolved_ip = resolve_dns_over_vpn(host)
    if not resolved_ip:
        raise OSError("隧道 DNS 失败，禁止系统 DNS 回退")
    host = security.public_ipv4(resolved_ip)

    err = None
    for res in socket.getaddrinfo(host, port, socket.AF_INET, socket.SOCK_STREAM, 0, socket.AI_NUMERICHOST):
        ...
        try:
            ...
            if "operation not permitted" in str(e).lower() or e.errno == 1:
                err = OSError(f"[错误代码 3006] [ERR_PROXY_BIND_TUN_PERM_DENIED] 绑定当前 VPN 网卡 失败，权限不足！必须以 root 权限运行，或者进程缺少 CAP_NET_RAW 权限。")
            elif "no such device" in str(e).lower() or e.errno == 19:
                err = OSError(f"[错误代码 3004] [ERR_ROUTE_DEV_NOT_FOUND] 绑定当前 VPN 网卡 失败，找不到设备！这通常是因为 OpenVPN 核心未能成功连接或已被异常终止。")
            if sock is not None:
                sock.close()
    if err is not None:
        raise err
