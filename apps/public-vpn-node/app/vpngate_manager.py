# Modified 2026-09-24 by public-vpn-node; GPL-3.0-or-later. See docs/源码审查.md.
#!/usr/bin/env python3
...
def print(*args, **kwargs):
    pass  # Raw upstream diagnostics are intentionally suppressed.
...
class DualStackHTTPServer(ThreadingHTTPServer):
    ...
    def handle_error(self, request, client_address):
        pass  # Do not emit arbitrary request exceptions to container logs.
...
def load_ui_config() -> dict[str, Any]:
    with lock:
        ...
        # Administrator-owned read-only secret is authoritative on every load.
        credentials = security.load_credentials()
        config.update({key: credentials[key] for key in ('username', 'password', 'secret_path')})
        config.update(host=UI_HOST, port=UI_PORT, proxy_port=LOCAL_PROXY_PORT, routing_ip_type="all")
        ...
        if not auth_file.exists() or updated:
            try:
                DATA_DIR.mkdir(exist_ok=True, parents=True)
                auth_file.write_text(json.dumps({k: v for k, v in config.items() if k not in security.SECRET_FIELDS}, ensure_ascii=False, indent=2), encoding="utf-8")
            except Exception:
                pass
...
def log_to_json(level: str, module: str, message: str) -> None:
    # Raw VPN and request messages may contain secrets; no persistent raw log.
    return
...
def read_nodes() -> list[dict[str, Any]]:
    ...
    for item in raw:
        if not isinstance(item, dict):
            continue
        try:
            profile = security.sanitize_config(item.get("config_text", ""))
            host, port, proto = vpn_utils.parse_remote(profile)
            node = dict(item)
            node.update(config_text=profile, ip=security.public_ipv4(host),
                        remote_host=host, remote_port=port, proto=proto)
            node["id"] = safe_name(str(node.get("id", "node")))
            node["config_file"] = str(CONFIG_DIR / (node["id"] + ".ovpn"))
            clean.append(node)
...
def fetch_api_text(url: str | None = None, use_ssl_verify: bool = True) -> str:
    if url not in (None, API_URL) or not use_ssl_verify:
        raise ValueError("只允许固定 HTTPS API 与证书校验")
    # Refuse redirects, including HTTPS-to-HTTP downgrade.
    class NoRedirect(urllib.request.HTTPRedirectHandler):
        def redirect_request(self, req, fp, code, msg, headers, newurl):
            raise ValueError("节点 API 重定向被拒绝")
    opener = urllib.request.build_opener(urllib.request.ProxyHandler({}), NoRedirect())
    with opener.open(API_URL, timeout=12) as response:
        body = response.read(10 * 1024 * 1024 + 1)
        if len(body) > 10 * 1024 * 1024:
            raise ValueError("节点 API 响应过大")
...
def row_to_node(row: dict[str, str], config_text: str) -> dict[str, Any]:
    ...
    ip = security.public_ipv4(remote_host)
            ...
            try:
                ...
                for row in rows[:MAX_SCAN_ROWS]:
                    ...
                    if not encoded:
                        continue
                    try:
                        config_text = security.sanitize_config(decode_config(encoded))
...
def openvpn_command(config_file: str, route_nopull: bool, dev: str = "tun") -> list[str]:
    profile = Path(config_file)
    if profile.resolve().parent != CONFIG_DIR.resolve() or profile.is_symlink():
        raise ValueError("VPN 配置必须位于容器配置目录")
    profile.write_text(security.sanitize_config(profile.read_text(encoding="utf-8")), encoding="utf-8")
...
def kill_existing_openvpn_processes() -> None:
    # Child lifecycle is managed by this process and the container init.
    return
...
def run_openvpn_until_ready(config_file: str, keep_alive: bool, route_nopull: bool, timeout: int | None = None, dev: str = "tun") -> tuple[bool, str, subprocess.Popen[str] | None]:
    ...
    if keep_alive:
        try:
            vpn_runtime.begin(process)
        except Exception:
            stop_process(process)
            return False, "VPN safety gate failed", None
    ...
    def reader() -> None:
        ...
        for line in process.stdout:
            ...
            if keep_alive:
                try:
                    vpn_runtime.event(process, line_str)
                except Exception:
                    try:
                        vpn_runtime.revoke()
                    finally:
        ...
        if keep_alive:
            with vpn_runtime.LOCK:
                if vpn_runtime.owner is process:
                    vpn_runtime.revoke()
        if not startup_done[0]:
            lines.put(None)
...
def setup_policy_routing(interface: str) -> None:
    try:
        vpn_runtime.setup_policy_routing(interface)
    except Exception as exc:
        raise RuntimeError("VPN policy routing failed") from exc


def cleanup_policy_routing() -> None:
    # Keep the mark gate and policy in place until the next atomic activation.
    with vpn_runtime.LOCK:
        vpn_runtime.owner = None
        vpn_runtime.candidate = None
        vpn_runtime.revoke()
...
def connect_node(node_id: str) -> str:
    ...
    try:
        ...
        with lock:
            DATA_DIR.mkdir(exist_ok=True, parents=True)
            auth_file.write_text(json.dumps({k: v for k, v in ui_cfg.items() if k not in security.SECRET_FIELDS}, ensure_ascii=False, indent=2), encoding="utf-8")
        ...
        log_to_json("INFO", "VPN", f"节点 {node_id} 连接成功，当前 VPN 出口网卡已启用")
    ...
    <div class="modal-content">
      ...
      <form id="credentials_form" onsubmit="saveCredentials(event)">
        ...
        <div class="form-group" style="margin-bottom: 20px;">
          ...
          <input type="text" id="cred_suffix" class="input-field" required pattern="[A-Za-z0-9]+" placeholder="由管理员设置">
        </div>
    ...
    <div class="modal-content" style="max-width: 480px;">
      ...
      <form id="network_form" onsubmit="saveNetwork(event)">
        <div class="form-group" style="margin-bottom: 16px;">
          ...
          <input type="number" id="net_proxy_port" class="input-field" readonly required min="1024" max="65535" placeholder="7928">
        </div>
        ...
        <div style="border-top: 1px dashed rgba(255,255,255,0.08); padding-top: 16px; margin-bottom: 16px;">
          ...
          <div class="form-group" style="margin-bottom: 16px;">
            ...
            <p>当前仅使用所有 IP；未启用第三方归属查询。</p>
            <input type="hidden" id="net_routing_ip_type" value="all">
            <div class="option-group" id="routing_ip_type_group" style="display:none">
...
function render(){
  ...
  const statusMessage = esc(state.last_check_message || "");
  ...
  if ($("status")) { $("status").innerHTML=`<span class="status-dot"></span>HTTP 代理本地接口：${esc(localProxy)} | 活动节点：${activeNodeInfo} | 状态：${statusMessage}`; }
...
function handleRoutingModeChange(mode) {
  ...
  if (mode === "fixed_region") {
    ...
    warningDiv.innerHTML = `⚠️ <strong>固定地区</strong>：限制仅连接选定国家的节点；后台仍测试全部候选节点。如果该国的所有可用节点都失效，会造成代理中断且<strong>绝不自动切换到其他国家</strong>的节点。`;
...
function openCredentialsModal() {
  alert("账号、密码及安全路径由管理员在本机凭据文件中维护；修改后重建容器。端口由部署配置固定。");
}
...
def check_proxy_health() -> dict[str, Any]:
    ...
    try:
        ...
        return {"ok": False, "error": "VPN 接口、防泄漏规则或策略路由未就绪"}
    ...
    def _curl_check_ip(url: str) -> dict[str, Any] | None:
        ...
        for p_host in proxy_hosts:
            ...
            try:
                auth_input = None
                if proxy_user is not None and proxy_pass is not None:
                    value = (proxy_user + ":" + proxy_pass).replace("\\", "\\\\").replace('"', '\\"')
                    auth_input = 'proxy-user = "' + value + '"\n'
                res = subprocess.run(cmd, input=auth_input, capture_output=True, text=True, timeout=6)
                if res.returncode == 0:
    ...
    try:
        ...
        return {"ok": False, "error": "出口连接测试失败 (api.ipify.org 无法经代理连通，可能是节点已失效或 VPS 防火墙限制了 UDP/TCP 出站端口)"}
    except Exception as e:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def log_message(self, format: str, *args: Any) -> None:
        pass  # Never log request paths, cookies, or attacker-controlled headers.
    ...
    def do_GET(self) -> None:
        ...
        if effective_path in ("/", "/index.html"):
            ...
            ovpn_ok = active_openvpn_running() and vpn_runtime.healthy()
            ovpn_err = ""
            ovpn_details = "未连接"
            if ovpn_ok:
    ...
    def do_POST(self) -> None:
        # Browser requests must be same-origin; non-browser clients may omit Origin.
        ...
        if effective_path == "/api/login":
            if not security.allow_login():
                self.send_json({"error": "请稍后重试"}, HTTPStatus.TOO_MANY_REQUESTS)
                return
            try:
                ...
                if expected_pwd and security.credential_match(input_pwd, expected_pwd) and security.credential_match(input_uname, expected_uname):
                    token = __import__("secrets").token_hex(32)
                    with lock:
                        now = time.time()
                        for old in list(active_sessions):
                            if active_sessions[old] <= now:
                                del active_sessions[old]
                        if len(active_sessions) >= 128:
                            active_sessions.clear()
                        active_sessions[token] = now + 8 * 3600
                    ...
                    self.send_header("Set-Cookie", f"session={token}; Path={cookie_path}; HttpOnly; SameSite=Strict; Max-Age=28800")
                    self.end_headers()
                    self.wfile.write(body)
                else:
                    self.send_json({"ok": False, "error": "用户名或密码不正确，请重新输入"}, HTTPStatus.FORBIDDEN)
            except Exception as exc:
                self.send_json({"ok": False, "error": "操作失败，请检查节点状态和配置"}, HTTPStatus.INTERNAL_SERVER_ERROR)
            return
        ...
        if effective_path == "/api/update_credentials":
            self.send_json({"error": "请由管理员修改挂载的凭据文件并重建容器"}, HTTPStatus.FORBIDDEN)
            return
        ...
        if effective_path == "/api/update_settings":
            try:
                ...
                try:
                    ...
                    if new_proxy_port_int != LOCAL_PROXY_PORT:
                        raise ValueError()
                except (TypeError, ValueError):
                    self.send_json({"ok": False, "error": "代理端口由部署配置固定为 7928"}, HTTPStatus.BAD_REQUEST)
                    return
                ...
                if routing_ip_type != "all":
                    self.send_json({"ok": False, "error": "当前版本仅支持所有 IP；未启用第三方归属查询"}, HTTPStatus.BAD_REQUEST)
                    return
        ...
        if effective_path == "/api/check":
            ...
            try:
                ...
                self.send_json({"ok": True, "node": {k: v for k, v in updated_node.items() if k != "config_text"}})
            except Exception as exc:
                self.send_json({"ok": False, "error": "操作失败，请检查节点状态和配置"}, HTTPStatus.INTERNAL_SERVER_ERROR)
...
def main() -> None:
    ...
    print("public-vpn-node: 管理服务已启动；凭据由管理员预配置。", flush=True)
